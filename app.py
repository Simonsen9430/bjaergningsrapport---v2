...
def admin():
    ...
    if request.method == 'POST':
        if 'add_user' in request.form:
            ...
            cur.execute("DELETE FROM users WHERE id=? AND is_admin=0", (request.form['delete_user'],))
        elif 'update_user' in request.form:
            cur.execute("UPDATE users SET password=? WHERE id=?", (request.form['new_pass'], request.form['update_user']))
...
def index():
    ...
    if request.method == 'POST':
        ...
        for t, d, f in zip(request.form.getlist('entry_time'), request.form.getlist('entry_desc'), request.files.getlist('entry_image')):
            filename = ''
...
def download_pdf(report_id):
    ...
    pdf.cell(200, 10, "Bjærgningsrapport", ln=True, align='C')
    pdf.ln(10)
    pdf.cell(200, 10, f"Sted: {report[1]}", ln=True)
    pdf.cell(200, 10, f"Opgave: {report[2]}", ln=True)
    pdf.cell(200, 10, f"Dato: {report[0][:16].replace('T', ' ')}", ln=True)
    pdf.ln(10)
    pdf.set_font("Arial", style='B', size=12)
    pdf.cell(200, 10, "Hændelsesforløb:", ln=True)
    pdf.set_font("Arial", size=11)
    for time, desc in entries:
        pdf.multi_cell(0, 8, f"{time} - {desc}", align='L')
        pdf.ln(1)
